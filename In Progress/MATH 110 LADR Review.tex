...
\begin{itemize}
    ...
    \begin{itemize}
    	...
    	\item Recall that $p(z)q(z) = q(z)p(z)$ for polynomials. It follows directly that $p(T)q(T) = q(T)p(T)$, because we only replace the symbol $z$ by $T$. Thus, when computing the polynomial of a linear map, the order of the factors does not matter.
    \end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item $q(T) = 0$ iff $q$ is a scalar multiple of the minimal polynomial $p$ of $T$.
