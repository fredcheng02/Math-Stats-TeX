...
\begin{itemize}
    ...
    Obviously $E(\lambda, T) \subseteq G(\lambda, T)$. When $\lambda$ is not an eigenvalue, $G(\lambda, T) = \{0\}$.
	...
	\item A operator $N$ is \textit{nilpotent} if $N^k = 0$ for some $k \in \Z^+$. A nilpotent operator cannot be injective/surjective. [Recall the composition of injective (surjective/bijective) functions must be injective (surjective/bijective).]
	\begin{itemize}
	\item The only eigenvalue of a nilpotent operator is 0. Proof is obvious.
	\end{itemize}
	\item Given nilpotent $N$, $N^{\d V} = 0$, again by the nullspace power chain.
