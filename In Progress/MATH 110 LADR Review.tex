...
\begin{itemize}
    ...
    \begin{enumerate}
    \item [(a)] is an abelian group under addition and
   	\item [(b)] has the multiplicative identity and distributive and associative properties in scalar multiplication.
    \end{enumerate}
...
\begin{itemize}
    ...
    \item A polynomial $p$ in an indeterminate $z$ with coefficients from a field $\F$ is the (not unique) \textbf{expression} $a_0 + a_1 z + \cdots + a_n z^n$ with all the $a_i$'s in $\F$. The \textit{degree} of a polynomial is the largest exponent of $z$ with nonzero coefficient, and if all the coefficients are 0, we call the polynomial the \textit{zero polynomial}. We customarily define the degree of the zero polynomial as $-\infty$ (as in this book), $-1$, or just undefined.
    
    P.S. The most general and rigorous definition of a polynomial over a ring (sometimes as an infinite formal sum for uniqueness) is not discussed here, because our focus is vector spaces over a field.
    \begin{itemize}
...
\newpage
\section{Polynomials}
\textit{Remark}. We used facts about polynomials in high school algebra. These facts were intuitively ``understood'' back then, but we never had the chance to rigorously prove them. An abstract algebra course delves deep into some materials on polynomials, which we still take for granted in this book. For example, note that when evaluating a polynomial $p(z)q(z)$ at $z = a$, although we still directly plug $a$ in and compute $p(a)q(a)$ to get the result, this is formally not rigorous. This ``plugging-in'' step requires the machinery of \textit{evaluation homomorphism} to be justified. As another example, one should show that for polynomials with coefficients from a field (more generally from an integral domain), $\deg p + \deg q = \deg pq$ holds, before using it.
...
\begin{itemize}
    \item For $T \in \LV$, the subspace $U$ of $V$ is \textit{invariant under $T$} if $T(U) \subseteq U$, or alternatively, $T|_U$ is an operator on $U$.
    ...
    \begin{itemize}
        \item As a special case, $T$ is not invertible iff 0 is an eigenvalue of $T$. This will become useful in Chapter 8.
    \end{itemize}
...
\begin{itemize}
    ...
    \item The \textit{orthogonal projection} $P_U$ of $V$ to its finite-dimensional subspace $U$ is the function $P_U: V \to U$ defined by $P_U(v) = u$, where $u \in U$ and $w \in U^\perp$ are the pair of \emph{unique} elements such that $v = u+w$. It is easy to verify $P_U \in \LV$ (note that $U \subseteq V$).
...
\begin{itemize}
    ...
    \item An operator $S \in \LV$ is an \textit{isometry} if $\nm{Sv}=\nm{v}$ for all $v \in V$. (an operator that preserves norm)
    \item There are many equivalent characterizations of isometry. The following are equivalent for $S \in \LV$:
    \begin{enumerate}[label=(\alph*)]
...
\end{itemize}
\item The \textit{singular values} of $T \in \LV$ are the eigenvalues of $\sqrt{T^*T}$, which each eigenvalue repeating $\d E(\lambda, \sqrt{T^*T})$ times.
...
\begin{itemize}
	\item This theorem introduces a new way to decompose an operator/matrix on an inner product space known as the \textit{singular value decomposition} (SVD). Replace $v$ in the equation above by each individual $e_j$, and we have $Te_j = s_jf_j$ for every $j$ between 1 and $n$. Thus we have
	\begin{equation*}
...
\begin{itemize}
	...
	\item A operator $N$ on $V$ is \textit{nilpotent} if $N^k = 0$ for some $k \in \Z^+$. A nilpotent operator cannot be injective/surjective. [Recall the composition of injective (surjective/bijective) functions must be injective (surjective/bijective).]
	\begin{itemize}
...
\begin{itemize}
    ...
    \item For operator $T$ on a complex vector space $V$ and its distinct eigenvalues $\lambda_1,\dots,\lambda_m$, we have
    \begin{enumerate}[label=(\alph*)]
    ...
    \item For an eigenvalue $\lambda$ of $T \in \LV$, its \textit{algebraic multiplicity} is defined as $\d G(\lambda,T) = \d \n (T-\lambda I)^{\dim V}$, and its \textit{geometric multiplicity} is defined as $\d E(\lambda,T) = \d \n (T-\lambda I)$. By default, when the book says ``multiplicity'' it means specifically the algebraic multiplicity.
    \begin{itemize}
        \item Since $V = G(\lambda_1,T) \oplus \cdots \oplus G(\lambda_m,T)$ for operator $T$ on a complex vector space $V$, $\d V$ is the sum of multiplicities of all eigenvalues of $T$.
    \end{itemize}
    \item A \textit{block diagonal matrix}, as its name tells, is a matrix of the form $\begin{bmatrix}
	A_1 & & 0 \\ & \ddots & \\ 0 & & A_m
    \end{bmatrix}$, where the $A_i$'s are square matrices on the diagonal and all other entries outside these $A_j$'s are 0.
    \item We give the block diagonal matrix decomposition theorem. Suppose $T$ is an operator over a complex vector space $V$. For the list of all distinct eigenvalues $\lambda_1, \dots, \lambda_m$ of $T$, each with corresponding multiplicity $d_1,\dots,d_m$. Then there exists a basis of $V$ with respect to which $T$ has a block diagonal matrix of the form $\begin{bmatrix}
	A_1 & & 0 \\ & \ddots & \\ 0 & & A_m
...
\begin{itemize}
    \item For an operator $T$ over a complex vector space $V$, let $\lambda_1, \dots, \lambda_m$ be all distinct eigenvalues of $T$, each with corresponding multiplicity $d_1,\dots, d_m$. Then the \textit{characteristic polynomial} of $T$ is defined to be \[(z-\lambda_1)^{d_1}\cdots (z-\lambda_m)^{d_m}.\]
    \begin{itemize}
    ...
    \begin{itemize}
        \item It follows directly that for an operator $T$ on a complex vector space, the characteristic polynomial must be a polynomial multiple of the minimal polynomial. (We have not defined the characteristic polynomials in the case $\F = \R$, but the result is similarly true. See the next chapter.) This is an important criterion for us to find the minimal polynomial of an operator.
    \end{itemize}
    \item The following result is another important criterion for finding the minimal polynomial. For operator $T$ on $V$, the zeros of the minimal polynomial $p$ are \textbf{precisely} the eigenvalues of $T$.
    ...
    For the other direction, suppose $\lambda \in \F$ is an eigenvalue of $T$ instead, then $\exists v \in V\backslash \{0\}$ such that $Tv = \lambda v$. We have used a number of times already that $T^j = \lambda^j v$ for arbitrary integer exponent $j$. Thus
    \begin{align*}
