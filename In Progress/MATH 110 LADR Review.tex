...
\begin{itemize}
    ...
    \item A \df{monic polynomial} is polynomial with highest-term coefficient being 1. (Here this definition applies to both real and complex vector spaces.)
    \item For $T \in \LV$, then there exists a unique monic polynomial $p$ of smallest degree such that $p(T)=0$. We call it the \df{minimal polynomial} of the operator $T$. Note that the minimal polynomial has real coefficients when $V$ is a real vector space, and has complex coefficients when $V$ is a complex vector space.
    ...
    \end{itemize}
    \item The following result is another important criterion for finding the minimal polynomial. For an operator $T$ on a vector space $V$, the zeros of the minimal polynomial $p$ are \textbf{precisely} the eigenvalues of $T$.
...
\textit{Remark}. In this section, $u,v$ by default denotes vectors in $V$, and $a,b$ are real numbers to represent an arbitrary complex number $a+bi$.
\begin{itemize}
    ...
    \begin{align*}
        0 = q(T_\C)u & = (a_0+b_0 i)u + (a_1+b_1 i)Tu + \dots + (a_{m-1} + b_{m-1} i)T^{m-1}u + T^mu \qquad [\text{equal to } q(T)u]\\ & = (a_0 + a_1T + \dots + a_{m-1}T^{m-1} + T^m)u + i(b_0 + b_1T + \dots + b_{m-1}T^{m-1})u
    \end{align*}
    ...
    P.S. We give another way of thinking about the proof. Consider $q$ instead as the the minimal polynomial of $T_\C$. If $b_0 + b_1z + \dots + b_{m-1}z^{m-1}$ is not a zero polynomial, then we can divide it by its highest-degree coefficient to get a monic polynomial $s(z)$ such that $s(T)u = 0$ for all $u \in V$. Therefore, $q$ must be of real coefficients [$q(z) \in \mathcal{P}(\R)$], and since we know the minimal polynomial of $T$ on $V$ is $p$, $q$ must be $p$.

    
    \item The multiplicity of $\lambda$ is equal to the multiplicity of $\conj{\lambda}$
\end{itemize}
