...
\begin{itemize}
    ...
    \end{itemize}
    \item The subspace dimension $\d U$ is always $\leq$ the whole space dimension $\d V$. To accomplish this we have to resort to the classic procedure: we add vectors from $U$ that are not in the span of the existing ones in $L$. Thus $L$ always remains linearly independent, and it should always be smaller in size than $B$, a spanning list of $U$. Our procedure will eventually terminate and no more vectors can be added to $L$, i.e., all the vectors in $V$ is in $\s(L)$. $L$ is our desired basis, and $|L| = \d U \leq \d V = |B|$.
    \begin{itemize}
...
\begin{itemize}
    ...
    \emph{It might make more sense to think about this in terms of the $(\cdot,\cdot)$ notation. The $(v_1,0),\dots,(v_n,0)$ spans the first slot, while the $(0,v_1),\dots,(0,v_n)$ spans the second slot in $V \times V$.}
...
\begin{itemize}
    ...
    \item We now prove a lemma that will be useful in describing all normal operators on real inner product spaces. Suppose $T$ is a normal operator on an inner product space $V$ (real or complex), and $U$ is a subspace of $V$ that is invariant under $T$, then
    \begin{enumerate}[label=(\alph*)]
        \item $U^\perp$ is invariant under $T$.
...
\begin{itemize}
    ...
    Conversely, we suppose that there is a orthonormal basis of $V$ with respect to which $\M(S)$ is of the special block diagonal form as stated in the theorem. For clarity we refer to the basis as $e_1,\dots,e_n$. From the block diagonal form, we know $V$ can be expressed as $U_1 \oplus \cdots \oplus U_m$, where each $U_j$ is of dimension 1 or 2, and $S$ is invariant under each $U_j$. It follows that for every $v \in V$, $v = u_1 + \dots + u_m$, where each $u_j \in U_j$. Note that all the $u_j$'s must be pairwise orthogonal, because the basis vector(s) of one $U$ is orthogonal to the basis vector(s) of another $U$. Note also that for every vector $v_j \in U_j$, if $\d U_j = 1$, then we have $Sv_j = v_j$ or $Sv_j = -v_j$. If $\d U_j = 2$, then given \[\M(S|_{U_j}) = \begin{bmatrix}
        \cos(\theta) & -\sin(\theta) \\ \sin(\theta) & \cos(\theta)
...
\begin{itemize}
    ...
    \item Similar matrices have the same trace by the claim above. Let $P = A^{-1}QA$, then \[\det P = \det(A^{-1}QA) = \det(QA^{-1}A) = \det Q\]
    \begin{itemize}
        \item In particular, for two bases $u_1,\dots,u_n$ and $v_1,\dots,v_n$ of $V$, \[\tr \M(T,(u_1,\dots,u_n)) = \tr \M(T,(v_1,\dots,v_n)).\]
    \end{itemize}
    \item By the theorem above, the trace of the matrix of an operator is irrespective of the basis chosen. \emph{We will soon see that the determinant of the matrix of an operator is irrespective of the basis chosen as well.} From the block diagonal decomposition of $T$ on a complex vector space, we may conclude that $\tr \M(T)$ should be the sum of eigenvalues of $T$ (repeated according to their multiplicities), which is exactly $\tr T$.
    ...
    \end{itemize}
    \item $\tr(S+T) = \tr S + \tr T$ for $S,T \in \LV$. This holds obviously if we look at $\M(S)$ and $\M(T)$ with respect to the same basis of $V$.
    \begin{itemize}
...
\begin{itemize}
    ...
    \end{bmatrix},\] $\det A$, the \df{determinant of the square matrix} $A$, is given by \[\sum_{(m_1,\dots,m_n) \in S_n} \sgn (m_1,\dots,m_n) A_{m_1,1} \cdots A_{m_n,n}.\]
