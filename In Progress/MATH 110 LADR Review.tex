...
\begin{itemize}
    ...
    (b)$\implies$(c): By the spectral theorems we have an orthonormal basis $e_1,\dots,e_n$ corresponding to nonnegative eigenvectors $\lambda_1,\dots,\lambda_n$. Construct $R \in \LV$ such that $Re_j = \sqrt{\lambda_j} e_j$ for every $e_j$. Then $R^2e_j = \lambda e_j = Te_j$, so that $R^2 = T$. Show that $R$ is a positive operator completes the proof.
    ...
    The idea is simple, for positive operator $T$, each eigenvalue $v$ of $T$ has $Tv = \lambda v$ for some nonnegative $\lambda$. Once we showed $Rv = \sqrt{\lambda}v$ for $R^2=T$, since the spectral theorem says that $V$ has a basis consisting entirely of these eigenvalues $v$ of $T$, we can show that $R$ is the unique square root.
    ...
    $$Tv = a_1 \lambda e_1 +\dots+ a_n \lambda e_n.$$ The two expressions above are equal, and since the $e$'s are a basis, we have $a_j(\lambda - \lambda_j) = 0$ for all $j$. Thus, 
    ...
    \end{itemize}
    \item For scalars $\lambda_1,\dots,\lambda_n$ all with absolute value 1 satisfying $Se_j = \lambda_j e_j$ for orthonormal basis $e_1,\dots,e_n$ of $V$ and $S \in \LV$, $S$ must be an isometry.
    ...
    Therefore, under $\C$, $S$ is an isometry is equivalent to saying that there is an orthonormal basis of $V$ consisting of eigenvalues of $S$ with corresponding eigenvalues all of absolute value 1.
\end{itemize}


\subsection{Polar Decomposition and Singular Value Decomposition}
\begin{itemize}
\item \S 7.3 tells us that there is a unique positive square root of $T$. We denote it by $\sqrt{T}$.
...
\end{itemize}
\item The \textit{singular values} of $T \in \LV$ are the eigenvalues of $\sqrt{T^*T}$, which each eigenvalue repeating $\d E(\lambda, \sqrt{T^*T})$ times. Naturally the singular values should all be nonnegative because $\sqrt{T^*T}$ is the positive square root.
\item It quite naturally follows that singular values of $T \in \LV$ are the nonnegative square roots of the eigenvalues of $T^*T$, with each eigenvalue repeated $\d E(\lambda,T^*T)$ times.

$T^*T$ is positive, so by the spectral theorem there is an orthonormal basis $e_1,\dots,e_n$ and nonnegative $\lambda_1,\dots,\lambda_n$ such that $T^*Te_j = \lambda_j e_j$ for every $j$. Thus, $\sqrt{T^*T}e_j=\sqrt{\lambda_j}e_j$, which we showed earlier in proving every positive operator has a unique positive square root.
\end{itemize}
