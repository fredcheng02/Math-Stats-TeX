...
\begin{itemize}
    ...
    \begin{proof}
        ...
        (Linear Independence) Now let $0 = \sum_{i,j} c_{ij} (\alpha_i \beta_j)$, with each $c_{ij} \in F$. Note that \[\sum_{i,j} c_{ij} (\alpha_i \beta_j) = \sum_j \left(\sum_i c_{ij}\alpha_i\right)\beta_j,\] by the linear independence of $\beta_j$'s over $E$ we get that $\sum_i c_{ij}\alpha_i = 0$. Again by the linear independence of $\alpha_i$'s over $F$ we have all $c_{ij}$'s are 0.
    \end{proof}
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        (Proof 1, from Dummit and Foote) Let $L$ be any finite field of prime characteristic $p$. At the beginning of this section we first showed $|L| = p^n$ for some $n \in \Z^+$, which implies that $L$ must be in some splitting field $K$ of $x^{p^n}-x \in \Z_p[x]$. Since by the last paragraph that $|K| = p^n$, $L=K$. Since all splitting fields $K$ of $x^{p^n}-x$ over $\Z_p$ are unique up to isomorphism, all finite fields of characteristic $p$ are unique up to isomorphism with the splitting field of $x^{p^n}-x$ over $\Z_p$. \qed
