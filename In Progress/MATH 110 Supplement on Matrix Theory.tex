...
\makeatother
\begin{itemize}
    \item You should know that a real matrix $A$ with $A^{-1} = A^t$ is called an \df{orthogonal matrix}, and a complex matrix $A$ with $A^{-1} = A^*$ is called a \df{unitary matrix}, by the equivalence between square matrices and operators, and between $S^{-1} = S^*$ and an isometry $S$.
\end{itemize}
