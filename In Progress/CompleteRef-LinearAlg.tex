...
\begin{rem*}
    In Sections 1 to 5 below, we assume by default that $V$ is a finite-dimensional vector space (FDVS) over $\F$.
\end{rem*}
...
\begin{namedthm}[Rank-nullity theorem]
    Let $V$ be finite-dimensional and $T\in \LVW$, then $\ran T$ is finite-dimensional and \[
        \dim V = \nullity T + \rank T.
    \]
\end{namedthm}
...
\begin{defn}
    On the vector space of matrices $\operatorname{Mat}_{m\times n}(\F)$, we can define a canonical inner product by \[
...
\begin{fact}
    The operator norm of a matrix $A\in \operatorname{Mat}_{m\times n}(\F)$ \[
