...
\begin{itemize}
    ...
    \begin{itemize}
        \item The theorem above tells us that two affine subset to $U$ are either the same or  disjoint. In particular, for any $v \in U$, $v+U$ is parallel to $U$ and is thus not a basis vector for $V/U$.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{equation*}
        ...
        \right.
    \end{equation*}
    with the Kronecker delta. These $\phi_i$'s are designed to form a basis of $V'$. It suffices to show that $\phi_1,\dots,\phi_n$ is linearly independent in $V'$. Consider $$a_1\phi_1+\dots+a_n\phi_n=0$$. Since $(a_1\phi_1+\dots+a_n\phi_n)(v_i)=a_i$, all $a_i$'s are 0.
    \begin{itemize}
    ...
    \begin{itemize}
        \item first, every $\psi \in V'$ can be written as $\sum_{i=1}^n \psi(v_i)\phi_1$. Since $(\sum_{i=1}^n \psi(v_i)\phi_1)(v_j) = \psi(v_j)$, the sum and $\psi$ agrees on all basis vectors and they are equal.
    ...
    \item As expected $U^0$ is a subspace of $V'$. The zero linear functional takes $U \subseteq V$ to $0$. $(\phi+\psi)(U) = 0$ and $\lambda\phi(U) = 0$ is omitted as usual.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Conjugate symmetry gives us ``linearity" in the second slot: $$\langle u, \lambda v+w \rangle = \bar{\lambda} \langle u, v \rangle + \langle u,w \rangle;$$
