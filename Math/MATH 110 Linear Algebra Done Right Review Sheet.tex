...
\begin{itemize}
    ...
    \item Invertibility is equivalent to bijectivity between vector spaces. Linear maps are functions, so the proof is exactly the same.
    ...
    \end{itemize}
    \item Now we show the isomorphism between a linear map and its corresponding matrix. When the orders of the bases $\bv$ and $w_1,w_2,\dots,w_m$ are fixed, then $\M$ is a function from $\LVW$ to $\F^{m,n}$. As we have noted earlier $\M$ is linear. Thus, it suffices to show that $\M$ is bijective. A linear map and its matrix representation are homomorphic not merely with respect to linearity, but also with respect to the preservation of the order of the product $\M(ST) = \M(S)\M(T)$.
    \begin{itemize}
    ...
    \begin{equation*}
        ...
        \end{bmatrix},
    \end{equation*}
    where $a_1,\dots,a_n$ are given by $v = a_1v_1+\cdots+a_nv_n$.
