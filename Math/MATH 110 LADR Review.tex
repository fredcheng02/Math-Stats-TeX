...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        Clearly $T$ is a linear map, and we basically want to show $T$ is bijective. For $sq+r=0$, we must have $q=0$ and $r=0$ because otherwise $\deg sq \geq m > \deg r$. We proceed to figure out that $\d(\mathcal{P}_{n-m}(\F) \times \mathcal{P}_{m-1}(\F)) = n+1 = \d \mathcal{P}_n(\F)$.
    \end{itemize}
